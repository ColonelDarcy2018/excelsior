...
Java_com_itexcelsior_as_x_rtmp_ffmpeg_MainActivity_getDumpLog(JNIEnv *env, jobject thiz,
                                                              jstring _file_path) {
    ...
    if (ret < 0) {
        return reinterpret_cast<jstring>(avError(ret));
    }
...
Java_com_itexcelsior_as_x_rtmp_ffmpeg_MainActivity_printDeviceData(JNIEnv *env, jobject thiz) {

//    avdevice_register_all();
    //获取当前安卓设备上下文信息
//    AVFormatContext *fmt_ctx = avformat_alloc_context();

    //TODO 关闭 AVFormatContext *fmt_ctx
}
...
Java_com_itexcelsior_as_x_rtmp_ffmpeg_MainActivity_pushRtmpFile(JNIEnv *env, jobject thiz,
                                                                    jstring path_) {
    ...
    //输出的地址
    const char *outUrl = "rtmp://49.232.93.219/live/test";
